...
func (h WeTest) Object_Post_Delete(this string, t TestAction, result func(err string)) {
	const a = "Object_Post_Delete "

	h.Log(this + a + h.obj.ObjectName)

	// enviamos la petición al servidor para contar los elementos
	h.SendOneRequest("POST", "delete", h.obj.ObjectName, t.Data, func(serverResp []map[string]string, err string) {
		...
		h.Log("TEST POST DELETE DATA RESPUESTA SERVIDOR:", serverResp)
